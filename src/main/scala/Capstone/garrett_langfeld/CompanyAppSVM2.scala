...
object CompanyAppSVM2 extends App{
...
*/
  ...
  def crossValidate(indust:Int=10) : Unit = {
    ...
    var pred0 = 0.toDouble
    var pred1 = 0.toDouble
    var pred2 = 0.toDouble
    var pred3 = 0.toDouble
    var pred4 = 0.toDouble
    ...
    for (k <- 0 until 10) {
      ...
      */
      ...
      //else if (j == s/10){
        ...
        /*
        var i1 = trainData1.iterator()
        var i2 = trainData2.iterator()
        while (i1.hasNext){
          trainData.add(i1.next())
        }
        while (i2.hasNext){
          trainData.add(i2.next())
        }
        */
        //List<String> trainData_3 = new CopyOnWriteArrayList<String>();
        trainData1.addAll(trainData2)
        trainData = trainData1
        //trainData.addAll(trainData1)
        //trainData.addAll(trainData2)
      }
      ...
      CompanyClassifier2.CompanyClassifierSVM2.learn(10)
      CompanyClassifier2.CompanyClassifierSVM2.test(testData)
      //CompanyClassifier2.CompanyClassifierSVM2Boost.test(allDataCopy)


      val predicted_0 = testData.filter(x => SVM2Prediction(x) == "0")
      val actual_0 = testData.filter(x => rating(x) == "0")
      val not_0 = testData.filter(x => rating(x) != "0")
      tp0 += actual_0.filter(x => SVM2Prediction(x) == "0").size
      tn0 += not_0.filter(x => SVM2Prediction(x) != "0").size
      fp0 += predicted_0.filter(x => rating(x) != "0").size
      fn0 += actual_0.filter(x => SVM2Prediction(x) != "0").size
      ...
      val predicted_1 = testData.filter(x => SVM2Prediction(x) == "1")
      val actual_1 = testData.filter(x => rating(x) == "1")
      val not_1 = testData.filter(x => rating(x) != "1")
      tp1 += actual_1.filter(x => SVM2Prediction(x) == "1").size
      tn1 += not_1.filter(x => SVM2Prediction(x) != "1").size
      fp1 += predicted_1.filter(x => rating(x) != "1").size
      fn1 += actual_1.filter(x => SVM2Prediction(x) != "1").size
      pred1 += predicted_1.size;
      act1 += actual_1.size;

      val predicted_2 = testData.filter(x => SVM2Prediction(x) == "2")
      val actual_2 = testData.filter(x => rating(x) == "2")
      val not_2 = testData.filter(x => rating(x) != "2")
      tp2 += actual_2.filter(x => SVM2Prediction(x) == "2").size
      tn2 += not_2.filter(x => SVM2Prediction(x) != "2").size
      fp2 += predicted_2.filter(x => rating(x) != "2").size
      fn2 += actual_2.filter(x => SVM2Prediction(x) != "2").size
      pred2 += predicted_2.size
      act2 += actual_2.size

      val predicted_3 = testData.filter(x => SVM2Prediction(x) == "3")
      val actual_3 = testData.filter(x => rating(x) == "3")
      val not_3 = testData.filter(x => rating(x) != "3")
      tp3 += actual_3.filter(x => SVM2Prediction(x) == "3").size
      tn3 += not_3.filter(x => SVM2Prediction(x) != "3").size
      fp3 += predicted_3.filter(x => rating(x) != "3").size
      fn3 += actual_3.filter(x => SVM2Prediction(x) != "3").size
      pred3 += predicted_3.size
      act3 += actual_3.size

      val predicted_4 = testData.filter(x => SVM2Prediction(x) == "4")
      val actual_4 = testData.filter(x => rating(x) == "4")
      val not_4 = testData.filter(x => rating(x) != "4")
      tp4 += actual_4.filter(x => SVM2Prediction(x) == "4").size
      tn4 += not_4.filter(x => SVM2Prediction(x) != "4").size
      fp4 += predicted_4.filter(x => rating(x) != "4").size
      fn4 += actual_4.filter(x => SVM2Prediction(x) != "4").size
      pred4 += predicted_4.size
      act4 += actual_4.size

      CompanyClassifier2.CompanyClassifierSVM2.forget()
    }
    ...
    println("3:     " + precision_3 + " " + recall_3 + " " + f1_3 + " " + acc_3 + " " + pred3 + " " + act3)
    println("4:     " + precision_4 + " " + recall_4 + " " + f1_4 + " " + acc_4 + " " + pred4 + " " + act4)
...
*/
  ...
  */
  ...
  /*
  CompanyClassifier.CompanyClassifierSVM2.learn(3)
  CompanyClassifier.CompanyClassifierSVM2.test(testData)
  */

  /*
  CompanyClassifier.CompanyClassifierSVM22.learn(3)
  CompanyClassifier.CompanyClassifierSVM22.test(testData)
  */



  //CompanyClassifier2.CompanyClassifierSVM23.learn(5)
  //CompanyClassifier2.CompanyClassifierSVM23.test(testData)
  ...
  */
  ...
  //CompanyClassifier.CompanyClassifierSVM23.crossValidation(10)
